...
public class Pager implements Callable<Integer> {
  ...
  @Option(names = { "--verbose" }, description = "Display detailed execution information")
  private boolean verbose;

  @Override
  public Integer call() {
    if (verbose) {
      System.out.println("Starting page replacement simulation...");
    }
    if (frames <= 0) {
    ...
    if (verbose) {
      System.out.println("Loaded " + pageReferences.size() + " page references from " + traceFile);
    }
    ...
    if (verbose) {
      ...
      System.out.println("Different References: " + simulator.getPages().size());
      System.out.println();
    }

    // Exibir resultados
    result.printSummary(algorithm, simulator.getFrames());
